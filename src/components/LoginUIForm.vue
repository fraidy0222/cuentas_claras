...
const login = async () => {
  try {
    ...
    if (err.response?.status === 409) {
      toast.error(err.response.data?.message ?? "Ya tienes una sesión activa.");
          ...
          <FieldGroup>
            <div class="flex flex-col items-center gap-2 text-center">
              ...
              <p class="text-muted-foreground text-balance">
                Ingresa a tu cuenta de Cuentas Claras
              </p>
            </div>
            ...
            <Field>
              <Button
                type="submit"
                class="w-full"
                :disabled="authStore.isLoadingAuth"
              >
                <Spinner v-if="authStore.isLoadingAuth" class="mr-2" />
                {{
                  authStore.isLoadingAuth ? "Ingresando..." : "Iniciar sesión"
                }}
              </Button>
            </Field>
